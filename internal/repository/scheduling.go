...
func (s *Store) DueSymbols(ctx context.Context) ([]string, error) {
 ...
 WHERE c.active AND (p.attempted_at IS NULL OR p.attempted_at<now()-interval '24 hours')
 ORDER BY EXISTS(SELECT 1 FROM watchlist_companies w WHERE w.company_id=c.id) DESC,p.attempted_at NULLS FIRST,c.symbol LIMIT 3`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CalendarDue(ctx context.Context) (bool, error) {
	var due bool
	err := s.db.QueryRow(ctx, `SELECT NOT EXISTS(SELECT 1 FROM provider_sync_state WHERE provider='yahoo' AND resource='calendar:' AND attempted_at>now()-interval '6 hours')`).Scan(&due)
	return due, err
}
func (s *Store) IntradaySymbols(ctx context.Context) ([]string, error) {
 ...
 AND NOT EXISTS(SELECT 1 FROM provider_sync_state p WHERE p.provider='yahoo' AND p.resource='intraday:'||c.symbol AND p.attempted_at>now()-interval '1 hour')
